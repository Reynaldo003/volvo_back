...
from .constants import (
    ...
    GENERAL_OBLIGATORIOS_OK,
    GENERAL_OK_O_NA,
    PRUEBA_MANEJO_IDS,
)
from .models import ChecklistGeneralCalidad
from .serializers import ChecklistGeneralCalidadSerializer
...
def fondo_pdf(canvas, doc):
    ...
    canvas.drawRightString(ancho - 0.55 * cm, 0.35 * cm, "Checklist general de calidad Volvo")
    canvas.restoreState()
...
def header_pdf(obj, estilos):
    logo = crear_logo_pdf(ruta_logo_volvo(), max_width_cm=3.8, max_height_cm=1.0)
    derecha = [
        Paragraph("CHECKLIST GENERAL DE CALIDAD", estilos["titulo"]),
        Paragraph(f"Folio: {obj.id} &nbsp;&nbsp; Generado: {escape(ahora_formateado())}", estilos["mini"]),
    ]
...
def tabla_datos_generales(obj, estilos):
    cliente = obj.cliente
    data = [
        [
        ...
        [
            Paragraph("Revisión", estilos["label"]),
            Paragraph(texto_pdf(fecha_local(obj.fecha_hora_revision)), estilos["value"]),
            Paragraph("Asesor", estilos["label"]),
            Paragraph(texto_pdf(recortar(obj.asesor_servicio, 34)), estilos["value"]),
            Paragraph("Inspector", estilos["label"]),
            Paragraph(texto_pdf(recortar(obj.tecnico_inspector, 34)), estilos["value"]),
        ],
        [
            Paragraph("Placas", estilos["label"]),
            Paragraph(texto_pdf(obj.placas), estilos["value"]),
            Paragraph("VIN", estilos["label"]),
            Paragraph(texto_pdf(recortar(obj.vin, 28)), estilos["value"]),
            Paragraph("Modelo", estilos["label"]),
            Paragraph(texto_pdf(recortar(obj.modelo, 30)), estilos["value"]),
        ],
        [
            Paragraph("Kilometraje", estilos["label"]),
            Paragraph(texto_pdf(obj.kilometraje), estilos["value"]),
            Paragraph("Orden", estilos["label"]),
            Paragraph(texto_pdf(obj.orden_servicio), estilos["value"]),
            Paragraph("Estado", estilos["label"]),
            Paragraph(texto_pdf("Terminada" if obj.checklist_terminado else "Abierta"), estilos["value"]),
        ],
        [
            Paragraph("Prueba manejo", estilos["label"]),
            Paragraph(texto_pdf("Sí" if obj.requiere_prueba_manejo else "No"), estilos["value"]),
            Paragraph("Fecha/Hora", estilos["label"]),
            Paragraph(texto_pdf(f"{fecha_simple(obj.fecha_prueba)} {hora_simple(obj.hora_prueba)}"), estilos["value"]),
            Paragraph("KM prueba", estilos["label"]),
            Paragraph(texto_pdf(f"{obj.kilometraje_inicial or '—'} a {obj.kilometraje_final or '—'}"), estilos["value"]),
        ],
        [
            Paragraph("Gerente Servicio", estilos["label"]),
            Paragraph(texto_pdf(recortar(obj.gerente_servicio, 34)), estilos["value"]),
            Paragraph("PST", estilos["label"]),
            Paragraph(texto_pdf(recortar(obj.pst, 34)), estilos["value"]),
            Paragraph("Agencia", estilos["label"]),
            Paragraph(texto_pdf(recortar(obj.agencia, 24)), estilos["value"]),
        ],
    ]
...
def tabla_checklist(obj, estilos):
...
def bloque_observaciones(obj, estilos):
    t = Table(
        [
            [Paragraph("OBSERVACIONES GENERALES", estilos["th"])],
            [Paragraph(texto_pdf(recortar(obj.observaciones, 700), "Sin observaciones."), estilos["value"])],
        ],
        colWidths=[19.6 * cm],
    )
    t.setStyle(TableStyle([
...
def bloque_evidencias_pdf(obj, estilos):
    evidencias = []

    for evidencia in obj.evidencias.all().order_by("creado"):
    ...
    elementos = [
        PageBreak(),
        Table(
            [[Paragraph("EVIDENCIAS FOTOGRÁFICAS", estilos["th"])]],
            colWidths=[19.6 * cm],
            style=TableStyle([
...
def firmas(estilos):
    t = Table(
        [
            ["", "", ""],
            [
                Paragraph("TÉCNICO / INSPECTOR<br/><font size='5'>Nombre y firma</font>", estilos["firma"]),
                Paragraph("GERENTE DE SERVICIO<br/><font size='5'>Nombre y firma</font>", estilos["firma"]),
                Paragraph("PST<br/><font size='5'>Nombre y firma</font>", estilos["firma"]),
            ],
        ],
...
def generar_pdf_checklist_general(obj):
    estilos = estilos_pdf()
    ...
    story = [
        ...
        Spacer(1, 6),
        tabla_checklist(obj, estilos),
        Spacer(1, 7),
        bloque_observaciones(obj, estilos),
        Spacer(1, 12),
        firmas(estilos),
    ]

    story.extend(bloque_evidencias_pdf(obj, estilos))

    return pdf_response(story, f"checklist_general_calidad_{obj.id}.pdf")

def validar_general_para_cierre(obj):
    checklist = obj.checklist or {}
    if not isinstance(checklist, dict):
        checklist = {}
    ...
    ids_a_validar_ok = set(GENERAL_OBLIGATORIOS_OK)
    if not obj.requiere_prueba_manejo:
        ids_a_validar_ok -= PRUEBA_MANEJO_IDS

    for item_id in sorted(ids_a_validar_ok):
        estado = str((checklist.get(item_id) or {}).get("estado") or "").strip().lower()
        if estado != "ok":
            pendientes.append(CHECKLIST_GENERAL_MAP.get(item_id, item_id))

    for item_id in sorted(GENERAL_OK_O_NA):
        estado = str((checklist.get(item_id) or {}).get("estado") or "").strip().lower()
        if estado not in {"ok", "na"}:
            pendientes.append(CHECKLIST_GENERAL_MAP.get(item_id, item_id))

    if obj.requiere_prueba_manejo:
        datos_prueba = {
            "Nombre del técnico o inspector": obj.tecnico_inspector,
            "Fecha de prueba": obj.fecha_prueba,
            "Hora de prueba": obj.hora_prueba,
            "Kilometraje inicial": obj.kilometraje_inicial,
            "Kilometraje final": obj.kilometraje_final,
            "Firma/Nombre de Gerente de Servicio": obj.gerente_servicio,
            "Firma/Nombre de PST": obj.pst,
        }
        for etiqueta, valor in datos_prueba.items():
            if not valor:
...
class ChecklistGeneralCalidadViewSet(viewsets.ModelViewSet):
    authentication_classes = [SignedUserAuthentication]
    permission_classes = [permissions.IsAuthenticated]
    parser_classes = [MultiPartParser, FormParser, JSONParser]

    queryset = (
        ChecklistGeneralCalidad.objects
        .select_related("cliente")
        .prefetch_related("evidencias")
        .all()
        .order_by("-creado")
    )
    serializer_class = ChecklistGeneralCalidadSerializer
    filter_backends = [OrderingFilter, SearchFilter]

    ordering_fields = [
        "creado", "actualizado", "fecha_hora_revision", "agencia", "asesor_servicio",
        "tecnico_inspector", "placas", "vin", "modelo", "kilometraje", "orden_servicio",
    ]
    search_fields = [
        "agencia", "asesor_servicio", "tecnico_inspector", "gerente_servicio", "pst",
        "placas", "vin", "modelo", "kilometraje", "orden_servicio", "observaciones",
    ...
    def update(self, request, *args, **kwargs):
        ...
        if instance.checklist_terminado:
            return Response({"detail": "Este checklist ya está terminado y no se puede editar."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def terminar(self, request, pk=None):
        ...
        if obj.checklist_terminado:
            return Response({"detail": "El checklist ya estaba terminado."}, status=status.HTTP_400_BAD_REQUEST)

        pendientes = validar_general_para_cierre(obj)
        if pendientes:
            return Response(
                {
                    "detail": "No se puede terminar el checklist. Faltan validaciones obligatorias de calidad.",
                    "pendientes": pendientes,
                },
                status=status.HTTP_400_BAD_REQUEST,
            )

        obj.checklist_terminado = True
        obj.fecha_terminado = timezone.now()
        obj.save(update_fields=["checklist_terminado", "fecha_terminado", "actualizado"])
    ...
    def checklist_pdf(self, request, pk=None):
        obj = self.get_object()
        return generar_pdf_checklist_general(obj)
